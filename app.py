...
html_title = """
<h1 style="font-weight:bold; padding:30px;border-radius:1px;">
...
result = df.groupby(by="YearMonth") ["TotalSales"].sum().reset_index()
result["Month_Year"] = result["YearMonth"].dt.strftime("%b'%y")
...
with view2:
    expander = st.expander("Montly Sales")
    data = result
    expander.write(data)
with dwn2:
    ...
    #Dual Chart - Bar graph & Line chart
